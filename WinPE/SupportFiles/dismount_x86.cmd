...
if not "%~dp0%mountdir%"=="" for /f %%e in ('dir "%~dp0%mountdir%\*" /b/a') do goto DISMOUNT
goto FULLERROR

:FULLERROR
echo.
echo %~dp0%mountdir% is empty! Have you run the relevant mount script for %arch%? Double check %~dp0%pemountdir%.
echo.
goto END

:DISMOUNT 
REM Gli apici in %enginedir% sono necessari perchè il path per Windows AIK contiene degli spazi.
call %~dp0"%enginedir%"\%enginefile% /UnMount-Wim /mountdir:%~dp0%mountdir% /commit
goto END
